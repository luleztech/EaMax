...
const DashboardSection = ({ onNavigate, refreshTrigger }) => {
  ...
  const confirmClearNotificationHistory = () => {
    Alert.alert(
      'Clear notifications',
      'Are you sure you want to delete all notifications in server and app?',
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            try {
              const result = await adminNotificationsAPI.deleteAllNotificationHistory();
              // Clear app state immediately so admin sees an instant reset.
              setRecentNotifications([]);
              setAllNotifications([]);
              await fetchExtraData();
              const verified = result?.verifiedEmpty === true;
              const clearedHistory = Number(result?.cleared_history || 0);
              const clearedScheduled = Number(result?.cleared_scheduled || 0);
              const clearedTotal = clearedHistory + clearedScheduled;
              showStatusModal(
                'info',
                'Cleared',
                verified
                  ? `Deleted ${clearedTotal} notifications from server and app history. You can now send new notifications from a clean state.`
                  : 'Delete request completed. History is refreshing now; if any row remains, tap Delete all once more.',
              );
            } catch (error) {
              ...
              const friendly =
                lower.includes('not found') || lower.includes('404')
                  ? 'Could not reach the clear-history endpoint. Deploy the latest backend (POST /api/admin/notifications/history/clear) or check your API URL and admin key.'
                  : raw || 'Could not clear notifications. Try again.';
              showStatusModal('info', 'Could not clear', friendly);
            } finally {
  ...
  const fetchDashboardStats = async () => {
    try {
      ...
      /** User counts from the API — always full digits (never k/m) so totals match the database. */
      const formatExactUserCount = (raw) => String(Math.round(Number(raw) || 0));
      /** Full TZS total from successful payments — exact digits, no k/m and no comma shortening */
      const formatRevenueTzsh = (raw) => {
        const whole = Math.round(Number(raw) || 0);
        return `TSh ${whole}`;
      };
      ...
      setStats([
        {
          title: 'Total Users',
          value: formatExactUserCount(data.totalUsers || 0),
          change: '+0%',
          subtitle: 'All active users',
          gradient: ['#2563eb', '#1e40af'],
          icon: 'account-group',
        ...
        {
          title: 'Premium Users',
          value: formatExactUserCount(data.premiumUsers || 0),
        ...
        {
          title: 'Revenue',
          value: formatRevenueTzsh(data.revenue ?? 0),
        ...
        {
          title: 'Expired Subs',
          value: formatExactUserCount(data.expiredSubscriptions || 0),
          change: '0%',
          subtitle: 'Need renewal',
          gradient: ['#ef4444', '#dc2626'],
          icon: 'alert-circle',
      ...
      <Modal
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.notificationsHistoryActions}>
              <TouchableOpacity
                style={[
                  styles.clearHistoryButton,
                  clearingNotifications && styles.clearHistoryButtonDisabled,
                ]}
                onPress={confirmClearNotificationHistory}
                disabled={clearingNotifications}
                activeOpacity={0.8}>
                {clearingNotifications ? (
                  <ActivityIndicator size="small" color="#fca5a5" />
                ) : (
                  <Icon name="delete-sweep" size={20} color="#f87171" />
                )}
                <Text style={styles.clearHistoryButtonText}>
                  {clearingNotifications ? 'Deleting…' : 'Delete all from server'}
                </Text>
              </TouchableOpacity>
            </View>
